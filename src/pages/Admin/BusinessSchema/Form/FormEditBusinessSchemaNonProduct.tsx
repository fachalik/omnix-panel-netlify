...
import { usePatchProductNonPlatform } from '../Hooks/useGetProduct';
...
export default function FormEditBusinessSchemaNonProduct({
  handleClose,
  data,
  changeKey,
}: IFormUsers) {
  ...
  const { mutate } = usePatchProductNonPlatform();

  const { form, isLoading, onFinish, onFinishFailed } = useFormBusinessSchema({
    handleCloseForm,
    mutate,
    data,
    changeKey,
