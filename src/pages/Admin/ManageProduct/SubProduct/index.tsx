...
import {
  useGetSubProduct,
  useDestroySubProduct,
} from './Hooks/useGetSubProduct';
...
import FormSubProduct from './Form/FormSubProduct';
import FormSubProductEdit from './Form/FormSubProductEdit';
...
export default function SubProduct() {
  ...
  const { mutate } = useDestroySubProduct();
  ...
  const { data, isLoading, isSuccess, isError, error }: any = useGetSubProduct({
  ...
  const columns: ColumnsType<any> = [
    ...
    {
      title: 'Sub Product Description',
      dataIndex: 'subproductDescription',
    },

    {
      title: 'Sub Product Unit',
      dataIndex: 'subproductUnit',
    },
    {
      key: 'subproductPrice',
      title: 'Price',
      ...
      render: (_, record: any) => {
        return (
          <p>{`${formatRupiah(record?.subproductPrice.toString(), 'Rp. ')}`}</p>
        );
      },
    },
    {
  ...
  return (
    <div>
      <Card>
        ...
        <div style={{ fontSize: 12 }}>Tambah Sub product</div>
        <Divider />
        <div
          style={{
            display: 'flex',
      ...
      >
        <FormSubProduct handleClose={handleCancelCreate} />
      </Modal>
      ...
      >
        <FormSubProductEdit handleClose={handleCancelEdit} data={editData} />
      </Modal>
    </div>
  );
}
